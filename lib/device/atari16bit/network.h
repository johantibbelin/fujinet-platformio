...
/**
 * Number of devices to expose via ACSI, becomes 0x71 to 0x70 + NUM_DEVICES - 1
 */
...
public:
    ...
    /**
     * Called for ACSI Command 'O' to open a connection to a network protocol, allocate all buffers,
     * and start the receive PROCEED interrupt.
     */
    virtual void open();

    /**
     * Called for ACSI Command 'C' to close a connection to a network protocol, de-allocate all buffers,
    ...
    /**
     * Process incoming ACSI command for device 0x7X
...
private:
    /**
     * ACSINet Response Buffer
     */
    uint8_t response[1024];

    /**
     * ACSINet Response Length
     */
    ...
    /**
     * The currently set Prefix for this N: device, set by ACSI call 0x2C
     */
    std::string prefix;
    ...
    /**
     * The channel mode for the currently open ACSI device. By default, it is PROTOCOL, which passes
     ...
     * @return TRUE on error, FALSE on success. Used to emit ACSInet_error or ACSInet_complete().
     */
